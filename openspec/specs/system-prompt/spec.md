...
定义系统提示的构建流程，包括模型专用 base prompt、环境信息注入、InstructionPrompt 加载、技能列表注入以及组装顺序。
...
系统 MUST 在系统提示中使用 verbose 模式（`<available_skills>` XML 格式）注入技能的名称、描述和 location 摘要（而非完整正文）。技能完整内容 MUST 通过 skill 工具按需加载。
